...
# Nhập lớp Ui_MainWindow từ file railfence.py đã được tạo từ railfence.ui
# Giả sử file railfence.py nằm trong thư mục 'ui'
from ui.railfence import Ui_MainWindow # Đảm bảo đường dẫn và tên file này chính xác
...
class MyApp(QMainWindow):
    ...
    def call_api_encrypt(self):
        # URL của API Flask cho mã hóa Rail Fence
        ...
        key_str = self.ui.txt_key.toPlainText() # Lấy key dưới dạng chuỗi

        try:
            key = int(key_str) # Chuyển đổi key sang số nguyên cho Rail Fence
            if key <= 1:
                QMessageBox.warning(self, "Invalid Key", "Key must be an integer greater than 1 for Rail Fence Cipher.")
                return

        except ValueError:
            QMessageBox.critical(self, "Invalid Key", "Key must be a valid integer.")
            return
        except Exception as e:
            QMessageBox.critical(self, "Key Error", f"An error occurred with the key: {e}")
            return
    ...
    def call_api_decrypt(self):
        # URL của API Flask cho giải mã Rail Fence
